  ...
  return (
    <div className="w-full overflow-hidden">
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        <Link to={`/course/${problem.course_slug}`} className="text-sm text-blue-400 hover:underline transition-colors hover:text-blue-300">
          ← Back to course
        </Link>
        <div className="mt-4 grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2">
            ...
            <div className="mt-4 rounded-xl border border-[var(--color-fa-border)] bg-[var(--color-fa-surface)] p-4">
              ...
              <div className="prose prose-invert prose-blue max-w-none mt-2 overflow-x-hidden">
                <pre className="whitespace-pre-wrap font-sans text-sm leading-relaxed text-slate-300 overflow-x-auto max-w-full">
                  {problem.description}
                </pre>
              </div>
            </div>
            <div className="mt-4">
              ...
              <textarea
                ...
                className="font-mono mt-2 min-h-[240px] w-full max-w-full resize-y rounded-lg border border-slate-600 bg-[#0a0f1a] p-3 text-sm text-slate-200 outline-none focus:border-blue-500 overflow-x-auto"
              />
              <button
                type="button"
              ...
              {submission && (
                <div className="mt-4 rounded-lg border border-slate-600 bg-[#0a0f1a] p-3 overflow-x-hidden">
                  ...
                  <pre className="mt-2 max-h-40 overflow-auto font-mono text-xs text-slate-400 max-w-full">
                    {submission.output}
                  </pre>
                </div>
              )}
          ...
          <div className="lg:col-span-1">
            <div className="rounded-xl border border-[var(--color-fa-border)] bg-[var(--color-fa-surface)] lg:sticky lg:top-8">
              <div className="border-b border-[var(--color-fa-border)] px-4 py-3">
                ...
                <p className="text-xs text-slate-500 overflow-x-hidden">
              ...
              <div className="max-h-[min(70vh,540px)] space-y-3 overflow-y-auto p-4">
                ...
                {messages.map((m, i) => (
                  <div
                    key={`${m.created_at}-${i}`}
                    className={`rounded-lg px-3 py-2 text-sm overflow-x-hidden ${
                      m.role === "user"
                  ...
                  >
                    ...
                    <div className="whitespace-pre-wrap word-break break-words">{m.content}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex gap-2 border-t border-[var(--color-fa-border)] p-3">
                <input
                  ...
                  className="min-w-0 flex-1 rounded-lg border border-slate-600 bg-[#0a0f1a] px-3 py-2 text-sm text-white outline-none focus:border-blue-500 overflow-x-hidden"
                />
                <button
                  type="button"
                  disabled={chatBusy}
                  onClick={onSendChat}
                  className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-500 disabled:opacity-50 transition-colors whitespace-nowrap"
                >
                  {chatBusy ? "…" : "Send"}
                </button>
              </div>
